...
export function ThreadDetail({ threadId, initialThread }: ThreadDetailProps) {
  ...
  const { data: messageHistory = [] } = useQuery<HistoryMessage[]>({
    queryKey: ["thread-messages", threadId],
    queryFn: async () => {
      return await api.get<HistoryMessage[]>(`/api/threads/${threadId}/messages`);
    },
    staleTime: Infinity,
  });
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      >
        {messageHistory.length === 0 && parts.length === 0 && !userMessage ? (
          <div className="flex h-full items-center justify-center">
            <div className="text-center max-w-md">
          ...
          <div className="space-y-4 max-w-4xl mx-auto">
            ...
            {messageHistory.map((msg, index) =>
              msg.role === "user" ? (
                <div key={`h-${index}`} className="flex justify-end">
